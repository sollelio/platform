...
Todo o encaminhamento vive num único ficheiro: `src/App.jsx` (140 linhas; `BrowserRouter` aberto em `src/App.jsx:54`, `<Routes>` em `src/App.jsx:61-134`). Não existe outro ficheiro de rotas — `src/main.jsx` (10 linhas) só monta `<App />` dentro de `<StrictMode>`; não há `createBrowserRouter` em lado nenhum de `src/`.
...
**Faixa de ambiente:** No `.env` do repositório, `VITE_APP_ENV=test` (linha 3) faz aparecer a faixa `<EnvBanner />` (`src/App.jsx:28`, `src/App.jsx:60`). O interruptor de manutenção que existia antes do router (`VITE_SITE_LOCKED` no `.env` + `src/pages/MaintenancePage.jsx`) foi **removido a 14/08/2026** — o router é agora incondicional.

| path | componente (ficheiro) | pública/autenticada | notas |
|---|---|---|---|
| `/` | `src/pages/FormEntryPage.jsx` | pública | `src/App.jsx:62`. Ecrã do código de acesso. Pré-preenche a partir de `?codigo=` (`src/pages/FormEntryPage.jsx:120-127`); em caso de código válido grava `sessionStorage.setItem("dlm_invite", …)` (`src/pages/FormEntryPage.jsx:145`) e navega para `/formulario` (`src/pages/FormEntryPage.jsx:146`). |
| `/formulario` | `src/pages/FormPage.jsx` | pública | `src/App.jsx:63`. Sem guarda de rota; guarda **dentro da página**: `src/pages/FormPage.jsx:397-414` — sem `sessionStorage["dlm_invite"]` faz `navigate("/")`, e também se o modelo não tiver `steps` (ausente, não-array, ou array vazio). |
| `/interesse` | `src/pages/CaptacaoPage.jsx` | pública | `src/App.jsx:66`. Comentário em `src/App.jsx:64-65`: «Porta pública do funil: o formulário de captação de interessados (sem código de acesso, fricção zero)». |
| `/admin/login` | `src/pages/LoginPage.jsx` | pública | `src/App.jsx:72`. Rota **irmã** da protegida, de propósito (comentário `src/App.jsx:67-71`). Com sessão já viva redirecciona: `if (sessao) return <Navigate to={destino} replace />` (`src/pages/LoginPage.jsx:234`), com `destino = destinoDepoisDoLogin(localizacao)` (`src/pages/LoginPage.jsx:140`; função em `src/lib/sessao.js:48-52`). Entrada por `supabase.auth.signInWithPassword` (`src/pages/LoginPage.jsx:161`). |
| `/__preview_aviso` | elemento inline em `src/App.jsx:73-80` que envolve `src/components/admin/AvisoDataDoEvento.jsx` | pública | Rota de pré-visualização de um componente de backoffice, **sem `ProtectedRoute`**. |
| `/admin` | — (redirecionamento) | pública (é só um `Navigate`) | `src/App.jsx:83`: `<Navigate to="/admin/inicio" replace />`. |
| `/admin/:separador/:p1?/:p2?` | `src/pages/AdminPage.jsx` | **autenticada** | `src/App.jsx:95-102`, envolvida em `<ProtectedRoute>`. Backoffice inteiro numa só rota; a tradução slug↔id do separador está em `src/lib/rotasAdmin.js` (`SLUG_POR_ID`, `src/lib/rotasAdmin.js:28-41`). Slug desconhecido → `<Navigate>` interno para `caminhoDeSlugAntigo(...)` ou `caminhoDoSeparador(SEPARADOR_POR_OMISSAO)` (`src/pages/AdminPage.jsx:874-882`). |
| `/evento/:id/:aba?` | `src/pages/EventoPage.jsx` | **autenticada** | `src/App.jsx:105-112`, envolvida em `<ProtectedRoute>`. |
| `/briefing/:id` | `src/pages/BriefingPage.jsx` | pública | `src/App.jsx:113`. **Sem `ProtectedRoute`** e sem token: lê por `id` (uuid) via `supabase.rpc("formulario_briefing", { p_id: id })` (`src/pages/BriefingPage.jsx:472`) e `supabase.rpc("briefing_materiais", { p_id: id })` (`src/pages/BriefingPage.jsx:503`). Ambas as RPC são `SECURITY DEFINER` com `grant execute … to anon, authenticated` (`docs/migracoes/020_rpcs_formularios_publicos.sql:358`; `docs/migracoes/031_briefing_materiais.sql:55`). |
| `/contribuir/:token` | `src/pages/ContribuirPage.jsx` | pública | `src/App.jsx:116`. Ver secção 6. |
| `/acompanhar/:token/:vista?/:sub?` | `src/pages/PortalPage.jsx` | pública | `src/App.jsx:127`. Ver secção 6. Valores literais de `:vista` tratados em `src/pages/PortalPage.jsx`: `"avaliar"` (:502), `"questionario"` (:516), `"sinal"` (:537), `"documentos"` (:556); sem `:vista` desenha a jornada. `:sub` chega aos filhos: na área dos documentos vai como `tipo={sub}` (`src/pages/PortalPage.jsx:568`) e vale `orcamento` / `contrato` / `proposta` (`src/components/portal/DocumentosVista.jsx:803`, `["orcamento", "contrato", "proposta"].map(...)`; assinatura em `DocumentosVista.jsx:1390`); no questionário vale `"responder"` ou `"respostas"` (`src/components/portal/QuestionarioVista.jsx:122`). |
| `/comunicado/:token` | `src/pages/ComunicadoPage.jsx` | pública | `src/App.jsx:132`. Ver secção 6. |
| `*` | `DestinoDesconhecido`, definida em `src/App.jsx:45-50` | pública | Fallback/404. |
...
Só duas rotas usam `ProtectedRoute`: `/admin/:separador/:p1?/:p2?` e `/evento/:id/:aba?` (`grep -rn "ProtectedRoute" src/` só devolve `src/App.jsx:13,98,100,108,110` e o próprio ficheiro). A protecção real dos dados está na base (RLS `to authenticated`, ex.: `docs/migracoes/049_portal_do_cliente_fase1.sql:74-79`), não no router.

### Fallback e redirecionamentos

- **404 / rota desconhecida:** `<Route path="*" element={<DestinoDesconhecido />} />` (`src/App.jsx:133`). `DestinoDesconhecido` (`src/App.jsx:45-50`) decide pelo prefixo: `pathname.startsWith("/admin") || pathname.startsWith("/evento")` → `<Navigate to="/admin/inicio" replace />`; caso contrário → `<Navigate to="/" replace />`. Sempre com `replace`.
- **`/admin` → `/admin/inicio`**, `replace` (`src/App.jsx:83`).
...
| item | valor |
|---|---|
| Rota | `path="/evento/:id/:aba?"` — `src/App.jsx:106` (elemento `<Route>` completo em `src/App.jsx:105-112`, com `<EventoPage />` dentro de `<ProtectedRoute>`, `:108-110`) |
...
| **pedido** | captação, lead | Sim — `CaptacaoForm.jsx:57` `textoBotao = "Enviar pedido"`; `CaptacaoPage.jsx:236` "Pedido recebido 🤍"; `FunilBoard.jsx:1088` "Novo pedido" | `PainelNovoFormulario.jsx:94` "Dados da captação"; `avisosAtualizacao.js:52` "cada lead no tempo certo"; `AvisoDataDoEvento.jsx:495` idem | [a] antigo persiste |
...
| **questionário** | formulário (p/ quem preenche), onboarding | Sim — `FormEntryPage.jsx:231` "Questionário do Evento"; `FormPage.jsx:714` "O vosso questionário foi submetido com sucesso."; `AdminPage.jsx:716` "O vosso questionário *Do Luxo à Mesa* está pronto."; `invites.js:126` "Este questionário já foi submetido…" | `avisosAtualizacao.js:114` "o formulário de onboarding preenchido pelo cliente"; `FormPage.jsx:612` `origem: "onboarding:markInviteUsed"`, `:639` `origem: "onboarding"` | [a] + [c] |
| **organizador** | casal, família, noivos | Sim — `EventTypesTab.jsx:115`; `EventTypeEditor.jsx:80` (comentário); a `MaintenancePage.jsx`, que também usava o termo, foi removida a 14/08/2026 | Só em campos de casamento (`exports.js:8-9,24,29,32`, `clientes.js:614-615`, `submissionFields.js:6-7`) — uso legítimo | [a] alinhado |
| **responsável no dia** | — | Sim — `FaixaOperacional.jsx:131` `rotulo="Responsável no dia"`; `src/data/formSteps.js:30` `label: "Nome do Responsável no Dia"`; `exports.js:22` `nome_responsavel: 'Responsável no Dia'` | — | [a] |
| **briefing** | — | Sim — `VisaoGeralEvento.jsx:758,764`; rota `/briefing/:id` (`App.jsx:113`); RPCs `formulario_briefing` e `briefing_materiais` (`BriefingPage.jsx:472,503`) | — | [a]+[c] |
...
| **contacto** (a pessoa) / **lead** | lead | «lead» **ainda visível** | `avisosAtualizacao.js:52`, `AvisoDataDoEvento.jsx:495` (visíveis); `CaptacaoForm.jsx:131,240`, `DashboardTab.jsx:270`, `LacunasFormulario.jsx:18` (comentários) | [a] antigo persiste |
...
| `briefing` (fica) | **Não existe tabela `briefing`.** É a rota `/briefing/:id` (`App.jsx:113`) + as RPCs `formulario_briefing` e `briefing_materiais` |
...
**A rota `/contribuir/:token` EXISTE** — está declarada em `src/App.jsx:116` e serve `src/pages/ContribuirPage.jsx`. Existem três páginas públicas por token, todas documentadas abaixo.

Inventário das rotas com `:token` (`grep ":token" src/App.jsx`): `/contribuir/:token` (`src/App.jsx:116`), `/acompanhar/:token/:vista?/:sub?` (`src/App.jsx:127`), `/comunicado/:token` (`src/App.jsx:132`). **Não existe** rota `/portal/:token` nem qualquer redirecionamento de um slug antigo para `/acompanhar` (comentário explícito em `src/App.jsx:121-122`; `grep -rn "\"/portal" src/` não devolve nada). O endereço do portal compõe-se em `src/lib/portal.js:396-397` (`` `${window.location.origin}/acompanhar/${token}` ``).
...
Botões da camada de interacção usam `1.5px` (`index.css:271`, `281`, `290`, `301`, `310`, `319`); pastilhas e alternadores usam `1px` (`:359`, `:376`). **Bordas tracejadas: 32 ocorrências em 19 ficheiros** — não são um caso isolado. Os papéis são: estado vazio (`1.5px dashed #E8DCC0`, `ComunicadoModelos.jsx:249`), o «+» de adicionar imagem (`1.5px dashed var(--gold)`, `CaptacaoForm.jsx:793`), engastes por preencher e blocos por rever (`FormularioDoEvento.jsx:638`,`:764`; `ConferenciaPeriodo.jsx:395`,`:621`,`:857`; `EventTypeEditor.jsx:376`,`:895`,`:1582`), avisos bloqueantes (`AvisosBloqueantes.jsx:179`, `AvisoDataDoEvento.jsx:162`, `AvisoMoradaDoEvento.jsx:149`), separadores de folha impressa (`imprimirConferencia.js:195`) e linhas de corte (`ComunicadoExpedicao.jsx:533`,`:1448`; `PagamentosEvento.jsx:1246`; `ClienteVista.jsx:252`; `GerarOrcamento.jsx:433`).
...
2. **Blocos `<style>{`…`}</style>` locais** — **20 ficheiros `.jsx`**. Nove deles são backoffice com prefixo `dlm-`, sempre justificados por um hover que o inline não dá (ex.: `ComunicadoModelos.jsx:181-195` — «Os hovers que os estilos em linha não dão (regra da camada de interacção)»; `MensagemEditor.jsx:175-185`; `ComunicadosTab.jsx:1364-1393`; `ComunicadoEditor`, `CentroNotificacoes`, `AvisoDataDoEvento`, `AvisoMoradaDoEvento`, `InicioTab`, `Navegacao`). Os outros onze não usam `dlm-`: são CSS de impressão e de documento (`BriefingPage`, `ComunicadoPage`, `documentos-pecas`, `GerarProposta`, `GerarOrcamento`, `GerarContrato`) ou selectores próprios (`FormPage`, `DocumentosLista`, `AvisosBloqueantes`, `PainelNovoFormulario`, `FormularioDoEvento`).
...
- `src/data/formSteps.js:18` e `:20` — tooltips «a nossa equipa pode entrar no espaço para montar» / «a nossa equipa recolher todo o material».
...
Não existe nenhum campo `responsavel`/`atribuido`/`assigned`/`operador` no sentido de pessoal. Os `nome_responsavel` / `contacto_responsavel` / `relacao_responsavel` que existem (`src/lib/exports.js:22-24` → rótulos «Responsável no Dia», «Contacto do Responsável», «Relação com os Noivos»; `src/lib/submissionFields.js:19-21`) são **respostas do cliente** dentro do JSONB `submissions.respostas` — o «Responsável no Dia» é uma pessoa do lado do cliente (a irmã, a wedding planner), não da casa.
...
| 23 | `src/lib/avisosAtualizacao.js:52` | `{ icon: "funil", nome: "Funil", sub: "cada lead no tempo certo" }` | «lead» está na tabela de abandonar (l. 493) e este ficheiro é **copy bloqueante** que a Nádia tem de reconhecer |
| 24 | `src/components/admin/AvisoDataDoEvento.jsx:495` | `subtituloDesbloqueado="recebe a data — cada lead no tempo certo"` | idem #23 |
| 25 | `src/lib/avisosAtualizacao.js:114` | `"…o que veio do formulário de onboarding preenchido pelo cliente…"` | «onboarding» está na tabela de abandonar (l. 494) e aparece em copy bloqueante |
...
| Rota `/comunicado/:token`, tabelas `comunicados`, `comunicado_destinatarios`, `comunicado_modelos`, RPCs `dlm_comunicado_*` (`publicar`, `retirar`, `ver`) | `App.jsx:132`, `comunicados.js:12,130,138,705` | `glossario.md:518-521` («mudar a rota partia os endereços já entregues») |
...
| «noivo/noiva/noivos» em campos e exportações de casamento | `exports.js:8-9,22-24,29,32`, `clientes.js:614-615`, `submissionFields.js:6-7` | `glossario.md:98-101` («podem aparecer *dentro* de um evento concreto — um casamento tem mesmo noivos») |
