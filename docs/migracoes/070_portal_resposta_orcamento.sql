-- ============================================================================
-- 070 · portal — a resposta ao orçamento chega à jornada
-- ============================================================================
-- O QUE ISTO RESOLVE
--
-- A cliente aceitava o orçamento (ou pedia uma alteração) e a jornada
-- continuava a cobrar: «O orçamento — está consigo, à espera de uma
-- resposta». A resposta vivia nos actos (057), mas a projecção de
-- dlm_portal_ver não a trazia — e a regra da casa é clara: se o portal
-- precisa de mais um dado, é SQL, não JavaScript.
--
-- Entra UMA chave nova, `resposta_orcamento`: o último acto sobre uma
-- publicação de orçamento deste evento — {acto, em} — ou NULL se não
-- houver. Só o gesto e o instante; nem nome, nem mensagem, nem valores.
-- O resto da função é o texto exacto da 069.
--
-- Correr DEPOIS da 069. Idempotente (create or replace).
-- ============================================================================
...
-- 1 · Num evento com orçamento publicado e aceite:
--   select public.dlm_portal_ver('<TOKEN>')->'resposta_orcamento';
--   -- Esperado: {"acto": "aceitou", "em": "<carimbo>"}.
--   -- E no portal: a pendência do orçamento cala-se; «o que está connosco»
--   -- passa a falar do sinal.

-- 2 · Num evento com orçamento publicado SEM resposta:
--   -- Esperado: null — e a pendência continua a pedir a resposta.
