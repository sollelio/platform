-- ============================================================================
-- 071 · o contrato assina-se ANTES do sinal — funil e jornada pela ordem real
-- ============================================================================
-- A DECISÃO (da Nádia, 02/08/2026)
--
-- O fluxo do negócio passa a ser:
--   interessado → orçamento → [Aceite] → CONTRATO → [assinado] →
--   SINAL (sempre 50%) → cliente → projecto (terminal do funil)
--
-- O contrato deixa de ser o fim da linha e passa a ser o fecho do negócio:
-- aceite o orçamento, segue o contrato para assinar; com ele assinado,
-- paga-se o sinal; com o sinal, a data é dela. A fase «contrato» passa a
-- ser o limbo pós-aceite (onde antes estava a fase «sinal»), e «sinal»
-- passa a significar «assinado, 50% por pagar».
--
-- TRÊS peças, numa transacção:
--   1 · Os dados existentes lidos à luz nova:
--       · fase 'contrato' (terminal antigo: projecto e contrato feitos)
--         → 'projecto' (o terminal novo);
--       · fase 'sinal' SEM contrato assinado (aceite, à espera — o limbo
--         antigo) → 'contrato' (o limbo novo). Quem já tem contrato
--         assinado fica em 'sinal', que agora diz exactamente isso.
--   2 · O invariante da 040 aperta: com o contrato antes do sinal, os
--       estados operacionais passam a exigir fase em (cliente, projecto)
--       — «contrato» já não é pós-sinal.
--   3 · dlm_portal_ver com a jornada pela ordem nova (contrato em 3.º,
--       sinal em 4.º) e as implicações de fase reescritas para a
--       semântica nova. Inclui tudo o que a 070 trouxe.
--
-- Correr DEPOIS da 070. Idempotente: os UPDATEs são condicionados, o
-- constraint recria-se por drop+add, o replace da função é replace.
-- ============================================================================

begin;

-- ── 1 · Os dados lidos à luz nova ──────────────────────────────────────────

-- O invariante antigo sai primeiro: ia proibir exactamente as escritas
-- que se seguem (um terminal antigo com status operacional a mudar de
-- fase). Volta já a seguir, apertado.
alter table public.submissions
  drop constraint if exists submissions_status_pos_sinal;

-- Terminais antigos (contrato DEPOIS do projecto): passam ao terminal
-- novo. Nada se perde — os documentos, actos e pagamentos ficam onde
-- estão; só a coluna do funil muda de nome.
update public.submissions
   set fase = 'projecto'
 where fase = 'contrato';

-- O limbo antigo (aceite, sinal pendente, contrato nem vê-lo): passa ao
-- limbo novo. Quem JÁ TEM contrato assinado fica em 'sinal' — que agora
-- diz exactamente a verdade: assinado, 50% por pagar.
   ...
   and not exists (
     select 1 from public.documentos d
      where d.submission_id = s.id
        and d.tipo = 'contrato'
        and d.assinado_em is not null
   );

-- ── 2 · O invariante da 040, apertado ──────────────────────────────────────
alter table public.submissions
  add constraint submissions_status_pos_sinal
  check (
    status not in ('Em Preparação', 'Confirmado', 'Concluído')
    or fase in ('cliente', 'projecto', 'perdido')
  );

commit;

-- ── 3 · A jornada pela ordem real ──────────────────────────────────────────
...
-- 1 · As fases migradas:
--   select fase, count(*) from submissions group by fase order by fase;
--   -- Esperado: ZERO em 'contrato' com projecto+contrato feitos (foram a
--   -- 'projecto'); os antigos «aguarda sinal» sem assinatura estão em
--   -- 'contrato'.

-- 2 · A jornada pela ordem nova:
--   select e->>'etapa' from jsonb_array_elements(
--     public.dlm_portal_ver('<TOKEN>')->'jornada') as t(e);
--   -- Esperado, por ordem: interessada · orcamento · contrato · sinal ·
--   -- projecto · preparacao · grande_dia.

-- 3 · Num evento pós-sinal (fase cliente), contrato e sinal aparecem FEITOS:
--   select e->>'etapa', e->>'estado' from jsonb_array_elements(
--     public.dlm_portal_ver('<TOKEN_CLIENTE>')->'jornada') as t(e);

-- 4 · O invariante aperta:
--   update submissions set status='Confirmado' where fase='contrato' and id='<X>';
--   -- Esperado: ERRO 23514 (contrato já não é pós-sinal).

-- 5 · A invariante de sempre:
