-- ============================================================================
-- 076 · a Preparação acende pelo TRABALHO, não pela fase
-- ============================================================================
-- O bug que a 075 desmascarou (apanhado pelo Hélio no telemóvel): com o
-- sinal a avançar a fase para Cliente sozinho, a etapa «Preparação»
-- acendia no mesmo instante — «compras feitas, listas fechadas» ao lado
-- de um questionário ainda por responder. A inferência pela fase
-- (cliente/projecto ⇒ preparação em marcha) vinha da 069 e sempre foi
-- preguiçosa; só nunca se viu porque a fase avançava tarde, à mão.
--
-- A Preparação passa a acender APENAS pelo que a prova: o questionário
-- entregue, ou o estado operacional que a Nádia marca («Em Preparação»/
-- «Confirmado»/«Concluído») — o mesmo critério da régua do backoffice.
-- Depois do sinal, o portal volta a dizer a verdade do roteiro §2.3:
-- «Onde estamos agora: A data reservada» · «A seguir: O projecto».
--
-- Uma linha muda; o resto é o texto exacto da 073. Correr DEPOIS da 075.
-- ============================================================================
...
-- 1 · Evento com sinal pago, questionário POR responder, status Recebido:
--   select e->>'etapa', e->>'estado' from jsonb_array_elements(
--     public.dlm_portal_ver('<TOKEN>')->'jornada') as t(e);
--   -- Esperado: preparacao = 'por_acontecer'. No portal: «Onde estamos
--   -- agora: A data reservada» · «A seguir: O projecto».

-- 2 · Entrega o questionário OU marca «Em Preparação» na ficha:
--   -- a etapa acende — agora com razão.
