-- 082_comunicados_no_portal.sql
--
-- O COMUNICADO NO PORTAL DO CLIENTE — fase 4, secção C.
--
-- Meses depois, a cliente quer reler as condições de montagem. Não vai à
-- procura da conversa do WhatsApp — vai ao portal, que é onde o evento
-- vive. Os dados já lá estão; esta migração abre a janela.
--
-- UMA alteração à dlm_portal_ver (cópia FIEL da versão da 078, que é a
-- que está a correr — o corpo veio do ficheiro, não de memória):
--   · a projecção ganha a chave 'comunicados': por comunicado, o TÍTULO,
--     o TOKEN (o endereço da folha) e a DATA DE ENVIO. Mais nada.
--   · entram só os enviados ÀQUELE evento (linha em
--     comunicado_destinatarios com enviado_em) e só enquanto a folha
--     está no ar — retirada ou expirada desaparece, não vira ligação
--     morta.
--   · NADA de ids, n_acessos, mensagens ou nomes. E o portal NÃO chama a
--     dlm_comunicado_ver: essa conta uma leitura por chamada, e cada
--     visita ao portal inflacionaria as leituras da folha. O portal
--     mostra a ligação; a leitura conta-se quando ela abrir a folha.
--   · os comunicados ficam FORA do portão do sinal, de propósito: a
--     folha é pública e reencaminhável, e já lhe foi entregue à mão na
--     conversa — escondê-la aqui não protege nada e faz o portal mentir
--     sobre o que ela já tem.
--
-- Requer a 081 (dispensado_em) e por isso corre DEPOIS da 080 e da 081.
-- Tudo o resto da função fica exactamente como estava: a mesma resposta
-- única para token inexistente/revogado/expirado, os mesmos grants.
...
begin
  ...
  -- morta. Fora do portão do sinal de propósito (ver o cabeçalho).
  select jsonb_agg(
           jsonb_build_object(
             'titulo',     c.titulo,
             'token',      c.token,
...
-- ═════════════════════════════════════════════════════════════
--
-- 1) A projecção nova não deixa sair nada a mais (com a chave anon, num
--    evento com um comunicado enviado):
--      select jsonb_pretty(dlm_portal_ver('<token do portal>') -> 'comunicados');
--      → cada elemento tem SÓ as chaves titulo · token · enviado_em
--
-- 2) Nenhum id vaza (o id do comunicado e o do destinatário):
--      select (dlm_portal_ver('<token>')::text ilike '%' || '<id do comunicado>' || '%')
--          or (dlm_portal_ver('<token>')::text ilike '%' || '<id do destinatário>' || '%')
--        as id_vazou;
--      → false
--
-- 3) Retirar a folha tira-a do portal (não vira ligação morta):
--      retirar o comunicado → dlm_portal_ver('<token>') -> 'comunicados'
--      → [] (e voltar a publicar volta a mostrá-la, com o MESMO endereço)
--
-- 4) Um destinatário POR ENVIAR ou DISPENSADO não aparece:
--      numa linha sem enviado_em (ou com dispensado_em) → fora do array
--
-- 5) Token inexistente/revogado/expirado continua a dar a MESMA resposta
