...
const nomeNovo = (sufixo) =>
  `foto_${Date.now()}_${Math.random().toString(36).slice(2, 8)}${sufixo}.jpg`;
...
const enviar = async (blob, sufixo) => {
  const caminho = nomeNovo(sufixo);
...
export const carregarFotografia = async (eventoId, ficheiro, { momento, ordem = 0 } = {}) => {
  ...
  const [p, g] = await Promise.all([enviar(pequena, "_p"), enviar(grande, "_g")]);
...
export const apagarFotografia = async (foto) => {
  ...
  const caminhos = [foto.caminho, String(foto.caminho || "").replace(/_p\.jpg$/, "_g.jpg")]
    .filter(Boolean);
  await supabase.storage.from(BALDE).remove(caminhos).catch(() => {});
};
